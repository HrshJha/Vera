...
# Model to use
LLM_MODEL = os.environ.get("GEMINI_MODEL", "gemini-3.5-flash")
...
class GeminiProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        candidate_models = []
        for m in [self.model, "gemini-3.5-flash-lite", "gemini-flash-lite-latest", "gemini-3.1-flash-lite", "gemini-3.1-flash-lite-preview", "gemini-3.5-flash"]:
            if m and m not in candidate_models:
                candidate_models.append(m)

        # 1. Try healthy keys in pool
        max_attempts = max(1, self.key_pool.total_keys)
        for _ in range(max_attempts):
            client, key_state = self.key_pool.get_client_and_key()
            ...
            for model_id in candidate_models:
                try:
                    ...
                    if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
                        continue  # Try next model candidate on this key
                    time.sleep(0.2)
            key_state.mark_429(30.0)

        # 2. Fallback to OpenRouter free models
        or_key = os.environ.get("OPENROUTER_API_KEY", "")
        if or_key:
            try:
                return OpenRouterProvider(or_key).complete(prompt, system)
            except Exception:
                pass

        raise RuntimeError("Failed to complete Gemini call across all keys, models, and OpenRouter")
...
class OpenRouterProvider(LLMProvider):
    def __init__(self, api_key: str, model: str = ""):
        self.api_key = api_key
        self.model = model or "google/gemini-2.0-flash-lite-preview-02-05:free"
    ...
    def complete(self, prompt: str, system: str = None) -> str:
            ...
            try:
                req = urlrequest.Request(
                    ...
                    data=json.dumps({"model": model_name, "messages": messages,
                                    "temperature": 0.2, "max_tokens": 1500}).encode("utf-8"),
                    headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json",
                             "HTTP-Referer": "https://magicpin.in", "X-Title": "magicpin AI Vera"}
                )
                resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
                data = json.loads(resp.read().decode("utf-8"))
                return data["choices"][0]["message"]["content"]
        ...
        raise RuntimeError("Failed to complete OpenRouter call across all candidate models")
...
class BotClient:
    ...
    def healthz(self):
        return self._request("GET", "/v1/healthz", 15)

    def metadata(self):
        return self._request("GET", "/v1/metadata", 15)

    def push_context(self, scope, cid, version, payload):
        return self._request("POST", "/v1/context", 15, {
...
class JudgeSimulator:
    ...
    def _warmup(self) -> bool:
        ...
        # Reset bot state for clean evaluation
        ...
        for cid, c in list(self.dataset.customers.items())[:10]:
